"""
Enhanced Fraud EDA, modeling, and Power BI export script.
Keeps original structure but adds:
- Business & model KPIs (for Power BI KPI cards)
- More detailed daily/hourly/user summaries
- Single kpi_summary.csv file for Power BI cards (no DAX needed)
"""
...
# === CORRELATION MATRIX ===
num_cols = ['amount', 'transaction_amount_log', 'transaction_hour', 'is_night_transaction',
            'transactions_per_user_last_24h', 'avg_amount_last_7days'] + [f'v{i}' for i in range(1, 29)]
...
# === MODELING ===
features = ['transaction_amount_log', 'transaction_hour', 'is_night_transaction',
            'transactions_per_user_last_24h', 'avg_amount_last_7days'] + [f'v{i}' for i in range(1, 29)]
X = df[features].copy()
...
# === Feature Descriptions (same as before) ===
feature_descriptions = [{'column':'id','description':'Unique row ID'},
